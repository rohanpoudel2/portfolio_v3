...
import { defaultMarker, Locations as locations } from '@/data/TravelledLocation'

const PopupContent: React.FC<{ title: string, description?: string }> = ({ title, description }) => (
  <div>
...
const Map: React.FC = () => {
  ...
  useEffect(() => {
    ...
    locations.forEach(location => {
      const marker = L.marker(location.position, { icon: location.icon || defaultMarker })
      const popupContent = ReactDOMServer.renderToString(<PopupContent title={location.title} description={location.description} />)
